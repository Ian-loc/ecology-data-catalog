...
$("#summary").innerHTML=[["Fontes catalogadas",all.length],["Áreas de pesquisa",new Set(all.flatMap(r=>split(r.research_areas))).size],["Com download gratuito",all.filter(r=>r.free_download==="sim").length],["Com API ou acesso automatizado",all.filter(r=>r.programmatic_access==="sim").length],["Com dados para o Brasil",all.filter(r=>["sim","parcial"].includes(r.covers_brazil)).length],["Com artigo revisado por pares",peer]].map(([k,v])=>`<div><strong>${v}</strong><span>${k}</span></div>`).join("");
